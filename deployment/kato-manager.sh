...
compose_cmd() {
    local cmd="docker compose -f $SCRIPT_DIR/docker-compose.yml"
    if [ -f "$SCRIPT_DIR/docker-compose.auth.yml" ]; then
        cmd="$cmd -f $SCRIPT_DIR/docker-compose.auth.yml"
    fi
    echo "$cmd"
}
...
case "$COMMAND" in
    ...
    setup-auth)
        ...
        AUTH_OVERRIDE="$SCRIPT_DIR/docker-compose.auth.yml"
...
ENVEOF
        ...
        # Generate docker-compose.auth.yml to pass API key to Qdrant service
        cat > "$AUTH_OVERRIDE" <<'AUTHEOF'
# Qdrant authentication override
# Generated by kato-manager.sh setup-auth
# Remove with: ./kato-manager.sh disable-auth
services:
  qdrant:
    environment:
...
AUTHEOF
        ...
        print_info "Qdrant auth override created at $AUTH_OVERRIDE"
        print_warn "Keep .env secure - it contains your database passwords!"
        print_info ""
        ...
        print_info ""
        print_info "Auth will be applied automatically on start/restart."
        print_info "To disable auth later: ./kato-manager.sh disable-auth"
        ;;

    disable-auth)
        AUTH_OVERRIDE="$SCRIPT_DIR/docker-compose.auth.yml"
        if [ -f "$AUTH_OVERRIDE" ]; then
            rm "$AUTH_OVERRIDE"
            print_info "Removed Qdrant auth override ($AUTH_OVERRIDE)"
            print_info "Qdrant will run without API key authentication on next restart."
            print_info ""
            print_info "Note: .env file still contains credentials for ClickHouse and Redis."
            print_info "To fully remove auth, also delete $SCRIPT_DIR/.env"
        else
            print_info "No auth override found - Qdrant auth is not enabled"
        fi
        ;;
