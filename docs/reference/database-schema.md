...
|---|---|---|---|
| `kb_id` | String | Knowledge base / node identifier (partition key) | `"node_weather_bot"` |
| `name` | String | Pattern SHA1 hash (matches `patterns_data.name`) | `"7729f0ed..."` |
| `emotives` | String (JSON list) | Rolling-window list of emotive dicts (default `'[]'`) | `[{"joy": 0.8}]` |
| `metadata` | String (JSON dict) | Set-union accumulator of tags/sources (default `'{}'`) | `{"source": "chat"}` |
| `entropy` | Nullable(Float64) | Pre-computed Shannon entropy (populated by finalize-training) | `2.317` |
