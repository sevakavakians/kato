...
# KATO FastAPI Manager Script
# Management script for FastAPI-based KATO architecture
# Usage: ./kato-manager-fastapi.sh [command] [options]
...
# Build Docker image
build_image() {
    log_info "Building KATO FastAPI Docker image..."
    
    cd "$KATO_ROOT"
    docker build -f Dockerfile.fastapi -t "$DOCKER_IMAGE" .
...
start_services() {
    ...
    if ! docker images | grep -q "kato.*fastapi"; then
        log_warning "Image not found, building..."
        build_image
    fi
    
    log_info "Starting KATO FastAPI services..."
