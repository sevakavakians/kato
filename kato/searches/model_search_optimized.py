"""
Optimized model search with fast pattern matching algorithms.
Maintains full backward compatibility while providing significant performance improvements.
"""
...
logger = logging.getLogger('kato.searches.model_search_optimized')
logger.setLevel(getattr(logging, environ.get('LOG_LEVEL', 'INFO')))


class OptimizedInformationExtractor:
    """
...
class OptimizedModelSearcher:
    ...
    def __init__(self, **kwargs):
        ...
        logger.info(f"OptimizedModelSearcher using {self.procs} CPUs")
        ...
        logger.info(f"OptimizedModelSearcher initialized: "
...
def create_model_searcher(**kwargs) -> Any:
    """
    Factory function to create appropriate model searcher.
    
    Uses environment variable KATO_USE_OPTIMIZED to determine which to use.
    
    Args:
        **kwargs: Arguments to pass to searcher constructor
        
    Returns:
        ModelSearcher instance (optimized or original)
    """
    use_optimized = environ.get('KATO_USE_OPTIMIZED', 'true').lower() == 'true'
    
    if use_optimized:
        logger.info("Using OptimizedModelSearcher")
        return OptimizedModelSearcher(**kwargs)
    else:
        # Import and use original if optimization disabled
        logger.info("Using original ModelSearcher")
