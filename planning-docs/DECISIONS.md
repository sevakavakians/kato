...
**Status**: **COMPLETE, VERIFIED, COMMITTED and PUSHED, FULLY RESOLVED.** Committed as `3706e73` "fix(ci): apply the ClickHouse schema one statement per request" on branch `main` (previous HEAD `80901c5`), pushed to `origin/main`. CI run `35650420692` on `3706e73`: Lint, "Initialise ClickHouse schema", and Import check all green; the unit-test step was still running as of this update. The planning-docs recording this fix were committed as `723fc3c` "docs(planning): record the CI ClickHouse schema-init fix", pushed to `origin/main`. **The Helm re-bootstrap question is also resolved**: the user confirmed no live Helm deployments of this chart exist as of 2026-09-21, so the latent bootstrap defect never affected a real deployment and no manual remediation is needed anywhere — see `project-manager/pending-updates.md`. See `planning-docs/completed/bugs/2026-09-21-ci-clickhouse-schema-init-multi-bug-fix.md` for full detail.
...
- **Risk**: Low on the fix itself (empirically verified against a real server, and CI ran green against it on Lint/schema-init/Import check). No operational risk remains — the pre-existing bootstrap Job's defect never touched a real deployment, since none existed while it was broken (user-confirmed).

### Open Items

1. ~~**Commit and push**~~ — DONE. Committed as `3706e73` on `main` (previous HEAD `80901c5`), pushed to `origin/main`. CI run `35650420692`: Lint, "Initialise ClickHouse schema", and Import check green; unit-test step still running as of this update. Planning-docs recording this fix committed as `723fc3c`, pushed to `origin/main`.
2. ~~**Re-run schema bootstrap against any existing Helm deployment**~~ — CLOSED, no action needed. User confirmed no live Helm deployments of this chart exist as of 2026-09-21, so bug #3 never actually applied to a real deployment. The fix is only meaningful for deployments made from this point forward, which will apply the schema correctly. This is closed because there was nothing to remediate, **not** because the old bootstrap logic turned out to work.
