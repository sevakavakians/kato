...
**Version**: **5.0.1** — released 2026-09-10 via `./container-manager.sh patch` (AUTO_MODE). Tag `v5.0.1` on `main` at `1481e44`; GitHub release [v5.0.1](https://github.com/sevakavakians/kato/releases/tag/v5.0.1); images `ghcr.io/sevakavakians/kato:5.0.1`/`:5.0`/`:5`/`:latest` (same digest `sha256:54c13094932b…`). Patch bump — a behavior-preserving round-trip elimination (the DECISION-018 metadata-sidecar fix, documented complete on 2026-09-09 but left uncommitted through v5.0.0, now committed as `ca8e47a`) plus example-client bug fixes; no API change. See DECISION-023 in `DECISIONS.md` and `completed/features/2026-09-10-kato-v5.0.1-release.md`. Prior release: **5.0.0** — 2026-09-09 via `./container-manager.sh major` (major bump for the breaking `anomalies`→`fuzzy_matches` prediction-field split, DECISION-019); see DECISION-022 and `completed/features/2026-09-09-kato-v5.0.0-release.md`. **Release gap (2026-09-10, not yet acted on)**: `main` is now well ahead of the released v5.0.1 image — it includes the observe-path deadlock fix (`9de98c3`) and the Phase 1.6 lock-free refactor (`b155cb5`, DECISION-026) that closed the Multi-Worker Uvicorn initiative, none of which are in any published image. The deployment stack runs an unreleased local build with these fixes. A release (5.0.2 patch, or 5.1.0) is warranted; see `project-manager/pending-updates.md` — version-bump choice left to the user.
...
**Test Coverage**: **482 passed / 4 skipped / 1 xfailed / 0 failed** (full suite — verified 2026-09-10 against the unreleased local build carrying the Phase 1.6 lock-free refactor, 651s; store-parity tool confirms 0 mismatched `kb_id`s). The single xfail is `test_concurrent_session_modifications`, the documented DECISION-024 same-session concurrent-write limitation (expected, not a regression). Up from the 475 passed / 4 skipped / 0 failed baseline verified against the released v5.0.1 image earlier the same day; the delta is the new Phase A store-cleanup tests plus the one intentional xfail — no unexpected failures introduced. `tests/tests/integration/test_worker_topology.py` passes 18/18 across `KATO_WORKERS` in {1, 2, 4}, including a new same-processor session-interleaving test added by Phase 1.6. The only known-intermittent item remains `tests/tests/api/test_monitoring_endpoints.py::test_metrics_collection_after_requests` (10s metrics-collection interval racing a 1s test sleep under multi-worker) — flaky by construction, unrelated to recent work. See `planning-docs/SPRINT_BACKLOG.md` and `planning-docs/completed/features/2026-09-10-phase-1.6-lock-free-refactor.md`
