...
**Status**: Stable and production-ready (maintenance mode). The deployed image (v6.0.1) contains everything through and including the recall-safe candidate bound and the `filter_pipeline` validation/messaging fixes. **One small piece of work is complete but uncommitted**: a user-raised single-event-vs-multi-event pattern grouping concern, investigated and closed as **NOT A BUG** (verified live — see DECISION-039), with a test-coverage gap, two doc/docstring defects, and one cosmetic API-message bug fixed along the way; 6 files uncommitted, low urgency (see `project-manager/pending-updates.md`). Five other items remain open pending a human decision (dependency upgrade, `REDIS_PASSWORD`, dashboard hardening, single-symbol fast-path semantics, `sort_symbols` bug); see `project-manager/pending-updates.md`.
