...
**Status**: Stable and production-ready (maintenance mode). **Uncommitted work in progress**: branch `chore/remediation-pass-1` (not merged) carries "Remediation Pass 1" — 7 live bug fixes (including a dead error-handling layer and a guaranteed-`TypeError` validation bug), SQL-injection parameterization, CORS/store-binding security hardening, performance quick-wins, ~10,100 lines of dead code removed, and a new CI workflow (ruff + bandit + unit tests, previously absent). Verified: full suite 588 passed / 3 skipped / 1 xfailed / 0 failed. Four items need a user decision before this can close (commit/merge, `requirements.lock` regeneration, orphan Redis key cleanup, a full stack recreate for the store-binding changes) — see DECISION-030 in `DECISIONS.md`, `completed/features/2026-09-16-remediation-pass-1.md`, and `project-manager/pending-updates.md`.
