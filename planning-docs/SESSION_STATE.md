...
*Last Updated: 2026-09-21 (CI ClickHouse schema-init fix — three bugs found and fixed, including a latent Helm production bug where the bootstrap Job never actually applied the schema. COMPLETE, VERIFIED, COMMITTED and PUSHED (`3706e73` on `origin/main`), FULLY RESOLVED — user confirmed no live Helm deployments exist, so the latent bug never affected a real deployment. See "Current Task" below.)*
