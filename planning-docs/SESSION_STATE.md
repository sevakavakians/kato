...
*Last Updated: 2026-09-21 (CI ClickHouse schema-init fix — three bugs found and fixed, including a latent Helm production bug where the bootstrap Job never actually applied the schema. COMPLETE, VERIFIED, COMMITTED and PUSHED (`3706e73` on `origin/main`). See "Current Task" below.)*

## Current Task
**CI ClickHouse Schema Init Failure Fixed — COMPLETE, VERIFIED, COMMITTED and PUSHED (2026-09-21).**
...
**Next immediate action**: because bug #3 means the Helm bootstrap Job has never actually run successfully, re-apply the schema against any existing real Helm deployment of this chart — this fix only corrects the chart code for installs/upgrades going forward, it does not retroactively repair an already-failed install. This remains an open human decision. See `project-manager/pending-updates.md`.
