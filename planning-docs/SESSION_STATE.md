...
*Last Updated: 2026-09-09 (worker-topology tests replace flaky multi-worker tests + worker_pid field added; DECISION-020; broadcaster fix flagged, not started)*

## Current Task
**No active task — worker-topology test replacement + worker_pid addition is COMPLETE (nothing committed yet; broadcaster-fix decision flagged, see `planning-docs/project-manager/pending-updates.md`). Pick next item from sprint backlog (Multi-Worker Uvicorn + Concurrent Training Safety is next queued — the new topology tests deterministically confirm part of what that initiative needs to fix).**
...
- **Open, flagged for human review (not decided)**: whether to fix the cross-worker broadcaster (Redis pub/sub) before the next release — see `planning-docs/project-manager/pending-updates.md`, alongside the still-open DECISION-019 major-version-bump question. Nothing from this work has been committed yet.
...
3. **Bug: Multi-worker (`KATO_WORKERS=4`) breaks websocket event delivery and concurrent session modification consistency** — P2 (discovered 2026-09-08, characterized during conftest.py FLUSHALL bug fix verification; **websocket-delivery half confirmed deterministically 2026-09-09**, see DECISION-020)
   ...
   - **2026-09-09 update**: `tests/tests/integration/test_worker_topology.py` now reproduces this deterministically (not intermittently) at `KATO_WORKERS=2` and `4` — 3 delivery tests fail every run with the specific missed worker pids named in the assertion; `KATO_WORKERS=1` passes 5/5. This also absorbs the former Root cause #3 websocket-timeout symptom (item 2 above), which was the same underlying gap.
   - `test_concurrent_session_modifications` loses half its concurrent writes (`assert 5 == 10`) — writes split across workers aren't consistently visible to each other. **Not re-verified 2026-09-09** (out of scope for the topology-test work; still tracked as open).
   ...
   - **Fix (Redis pub/sub fan-out or similar) not started** — flagged for human decision on priority before next release, see `planning-docs/project-manager/pending-updates.md`
