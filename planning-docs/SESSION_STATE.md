...
*Last Updated: 2026-09-22 (Single-event-vs-multi-event pattern grouping investigation — reported bug NOT reproduced, verified live against the running service; coverage gap and doc/docstring/API-message defects fixed instead. COMPLETE, UNCOMMITTED. See "Current Task" below.)*

## Current Task
**Single-Event vs. Multi-Event Pattern Grouping Investigation — COMPLETE, NOT A BUG, UNCOMMITTED (2026-09-22).**

**What happened**: User suspected KATO might conflate a pattern learned as ONE event with two symbols (`observe(["hello","world"])` → `learn()`) with the same symbols learned as TWO events (`observe(["hello"])`; `observe(["world"])` → `learn()`) — expecting the one-event pattern to report `world` in `missing` after observing `["hello"]`, and the two-event pattern to report it in `future`. Verified live against the running KATO 6.0.1 service (`localhost:8000`) with isolated `node_id`s: actual behavior matched the expectation exactly — one event gives `present=[["hello","world"]]`/`missing=[["world"]]`/`future=[]`/`confidence=0.5`; two events give `present=[["hello"]]`/`missing=[[]]`/`future=[["world"]]`/`confidence=1.0`. Both patterns coexist in one node and predict independently with no cross-talk; ClickHouse's stored `pattern_data` confirmed the distinct nesting directly. Both cases take the single-symbol fast path (`kato/workers/pattern_processor.py:1035`), which since `e0ee17d` (DECISION-028) shares the same `segment_by_alignment()` used by the general path. **Decision**: DECISION-039 in `planning-docs/DECISIONS.md`. **Closed as NOT A BUG.**

**Real defects found and fixed instead**: (1) a genuine test-coverage gap — this exact comparison, including the single-symbol-observation-against-single-event-pattern case, was never directly tested; added 4 tests to `tests/tests/unit/test_multi_symbol_event_predictions.py`. (2) Wrong docstrings in `kato/workers/pattern_processor.py:318` and `kato/workers/pattern_operations.py` (2 places) — claimed patterns "shorter than two events" aren't learned; the actual guard (`len(pattern) <= 1`) counts symbols, not events. Corrected. (3) Wrong examples in `docs/users/concepts.md` — one showed a prediction for an observation (`[['B']]` against `[['A'],['B'],['C']]`) that actually returns no prediction at all (fast-path first-token-only restriction); two others showed flat `missing`/`extras` instead of the actual nested, event-aligned form. Corrected against live output; added a note on event grouping. (4) `docs/reference/prediction-object.md` had one example with flat `missing` contradicting the nested form shown elsewhere in the same file. Corrected; added a note on the event-grouping distinction. (5) Cosmetic API bug: `kato/api/endpoints/sessions.py:504` built its learn-response message from `len(session.stm)` *after* the STM had been replaced with the post-learn remainder, so every learn reported "from 0 events" — fixed by capturing the count before learning. **Not yet verified at runtime** — the live deployment serves `ghcr.io/sevakavakians/kato:6.0.1` from `deployment/docker-compose.yml` and was not rebuilt/restarted (26h uptime, 191 active sessions at time of investigation).

**Verification**: full unit suite `./run_tests.sh --no-start --no-stop tests/tests/unit/` → **492 passed** in 319s. No product behavior changed apart from the API response message text.

**Commit status**: **UNCOMMITTED.** 6 files modified: `docs/reference/prediction-object.md`, `docs/users/concepts.md`, `kato/api/endpoints/sessions.py`, `kato/workers/pattern_operations.py`, `kato/workers/pattern_processor.py`, `tests/tests/unit/test_multi_symbol_event_predictions.py`.

**Next immediate action**: commit the working tree, then rebuild and redeploy to verify the cosmetic API-message fix at runtime (low urgency — cosmetic only). See `project-manager/pending-updates.md`.

**Archive**: `planning-docs/completed/bugs/2026-09-22-single-vs-multi-event-grouping-investigation-not-a-bug.md`. **Decision**: DECISION-039 in `planning-docs/DECISIONS.md`.

## Previous Task (context preserved)
**CI ClickHouse Schema Init Failure Fixed — COMPLETE, VERIFIED, COMMITTED and PUSHED, FULLY RESOLVED (2026-09-21).**

**What happened**: The "Unit tests" CI job has been failing since the CI workflow was added — "Initialise ClickHouse schema" exited with curl code 22 (latest failing run `35632623893`). Root-caused empirically against a real `clickhouse/clickhouse-server:24.8` container and found **three stacked bugs**: (1) CI POSTed the whole `init.sql` as one request, but ClickHouse's HTTP interface only executes one statement per request (`Code: 62`); GitHub Copilot's suggested `?multiquery=1` fix was tested and confirmed **not** to work (`Code: 115 UNKNOWN_SETTING` — `multiquery` is a `clickhouse-client` CLI flag, no HTTP equivalent exists); (2) the CI clickhouse service had no configured user, so every request failed `Code: 516 AUTHENTICATION_FAILED`; (3) **latent production bug** — the Helm chart's bootstrap hook (`charts/kato/scripts/bootstrap.py`) split `init.sql` on `;` and dropped every fragment starting with `--`, which silently collapsed all 8 statements (each sitting under a comment block) down to 3 broken ones. **The Helm bootstrap Job has never successfully applied this schema in any real deployment of the chart.** **Decision**: DECISION-038 in `planning-docs/DECISIONS.md`.

**What changed**: `config/clickhouse/init.sql` (+ its two byte-identical mirrors `deployment/config/clickhouse/init.sql`, `charts/kato/scripts/init.sql`) — all tables database-qualified (`kato.patterns_data` etc.), `USE kato;` removed (a per-statement HTTP applier has no session, so `USE` from one request never affects the next). New `scripts/apply_clickhouse_schema.py` — stdlib-only, waits for `/ping` with explicit timeout failure, splits and POSTs statements one at a time, surfaces ClickHouse's real error body on failure instead of a bare curl exit code. `.github/workflows/ci.yml` — schema step now runs the new script (`--wait 60`); clickhouse service gets `CLICKHOUSE_SKIP_USER_SETUP: '1'` (CI-only; compose uses `users.xml`, the chart uses secrets, neither needed a change). `charts/kato/scripts/bootstrap.py` — splitter replaced with a shared-rule, comment-aware `split_statements()`; kept in lockstep with the script above by a shared test rather than a shared import, since `bootstrap.py` is vendored into a Helm ConfigMap with no access to the rest of the repo. New `tests/tests/unit/test_clickhouse_schema_init.py` (9 tests) — both splitters yield all 8 statements, each table created exactly once, schema fully qualified with no `USE`, and the two `init.sql` mirrors match the canonical file byte-for-byte.
...
**Commit status**: **COMMITTED and PUSHED.** Commit `3706e73` "fix(ci): apply the ClickHouse schema one statement per request" on branch `main` (previous HEAD `80901c5`), pushed to `origin/main`. The commit contains exactly the 7 source/config/test files: `.github/workflows/ci.yml`, `charts/kato/scripts/bootstrap.py`, `charts/kato/scripts/init.sql`, `config/clickhouse/init.sql`, `deployment/config/clickhouse/init.sql`, `scripts/apply_clickhouse_schema.py`, `tests/tests/unit/test_clickhouse_schema_init.py` (planning-docs changes committed separately). The push triggered CI run `35650420692` (in progress as of this update; the Helm Chart workflow on the same SHA already passed).

**Next immediate action**: none remaining for this task. The Helm re-bootstrap question is resolved — the user confirmed no live Helm deployments of this chart exist as of 2026-09-21, so bug #3's defect never actually affected a real deployment; no manual schema re-application is needed anywhere. This is closed because no deployment existed to be harmed, **not** because the old bootstrap logic worked. The first deployment made from this fixed chart will apply the schema correctly. See `project-manager/pending-updates.md` (RESOLVED). Planning-docs recording this fix were themselves committed as `723fc3c` "docs(planning): record the CI ClickHouse schema-init fix", pushed to `origin/main`. CI run `35650420692` on `3706e73`: Lint, "Initialise ClickHouse schema", and Import check all green; the unit-test step was still running as of this update.

**Archive**: `planning-docs/completed/bugs/2026-09-21-ci-clickhouse-schema-init-multi-bug-fix.md`. **Decision**: DECISION-038 in `planning-docs/DECISIONS.md`.
