...
*Last Updated: 2026-09-16 (Remediation Pass 1 branch committed and merged to `main`; its four open items resolved; DECISION-031 closed a real prediction-ranking nondeterminism bug and removed a ProcessPoolExecutor pessimisation — see "Current Task" and "Previous Task" below)*
...
**Remediation Pass 1's four previously-open items are now RESOLVED** (commit/merge, `requirements.lock`, orphan Redis key cleanup, full stack recreate — see "Previous Task" below for detail). **Six pending human decisions remain** (see `planning-docs/project-manager/pending-updates.md` for full detail):
