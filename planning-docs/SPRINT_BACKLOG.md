...
**Status**: **COMPLETE, VERIFIED, COMMITTED and PUSHED, FULLY RESOLVED.** Committed as `3706e73` "fix(ci): apply the ClickHouse schema one statement per request" on branch `main` (previous HEAD `80901c5`), pushed to `origin/main`. Planning-docs recording this fix committed separately as `723fc3c`, also pushed. CI run `35650420692` on `3706e73`: Lint, "Initialise ClickHouse schema", and Import check all green; unit-test step still running as of this update. The Helm re-bootstrap question is also resolved — user confirmed no live Helm deployments of this chart exist, so the latent bootstrap defect never affected a real deployment. See DECISION-038 in `planning-docs/DECISIONS.md`.
...
**Next task**: none — fully resolved. No live Helm deployments of this chart exist as of 2026-09-21 (user-confirmed), so the latent bootstrap defect never affected a real deployment and no manual schema re-application is needed anywhere; the first deployment made from this fixed chart will apply the schema correctly. See `project-manager/pending-updates.md` (RESOLVED).
...
**Status**: **RELEASED and DEPLOYED** (2026-09-21). `perf/recall-safe-candidate-bound` merged to `main` (`51f8213`) and released as **v6.0.0** (MAJOR bump — version bump `419e695`, tag pushed, images `ghcr.io/sevakavakians/kato:6.0.0`/`:6.0`/`:6`/`:latest`); patched same day as **v6.0.1** (PATCH — version bump `e5a4cd6`, fix commits `c687268`/`23f13e9`, images `:6.0.1`/`:6.0`/`:6`/`:latest`). Deployment stack now pinned to v6.0.1. See DECISION-035/DECISION-036/DECISION-037 in `planning-docs/DECISIONS.md`.
**Files Modified (v6.0.0, same as the branch)**: `kato/filters/recall_bounds.py` (new), `kato/filters/executor.py`, `kato/filters/base.py`, `kato/searches/pattern_search.py`, `kato/config/settings.py`, `kato/sessions/session_config.py`, `kato/services/configuration_service.py`, `kato/sessions/session_manager.py`, `kato/sessions/redis_session_manager.py`, `kato/filters/rapidfuzz_filter.py`, `scripts/check_prediction_parity.py`, plus 2 new unit test files. **Files Modified (v6.0.1)**: `kato/sessions/session_config.py` (`VALID_FILTERS` constant), `kato/services/configuration_service.py`, plus the 4 pre-existing message sites listed below and 1 new unit test.

**Summary (v6.0.0 content, unchanged from the branch)**: ClickHouse cannot run KATO's scorer (`_lcs_ratio_scorer` — LCS-based; verified `arrayLevenshteinDistance` is a different metric with no LCS function available in ClickHouse 26.2). Instead, a **necessary-condition predicate** (a length window plus a token-overlap count, both computable by counting on stored `length`/`token_set` columns) is pushed into the ClickHouse query — provably never smaller than the true similarity, so it can only drop patterns that cannot possibly pass `recall_threshold`. ClickHouse never decides MATCH, only CANNOT; every survivor still runs the unchanged Python scorer. **Critical finding**: exact `Fraction` arithmetic is *unsafe* here (float(0.1) > 1/10 exactly, so an exact bound is stricter than the float scorer it approximates — 413 recall losses measured with the naive exact version, 0 with a deliberately weakened integer version). **Decisions shipped**: on by default with kill switch `KATO_RECALL_BOUND_ENABLED`; `LengthFilter` deleted entirely (its fixed ratios were recall-unsafe for `r<=2/3`, not just imprecise); `recall_threshold=0` rejected everywhere (the one case with no valid bound), which also closed two pre-existing validation gaps (`POST /sessions` unvalidated; a discarded boolean return silently defaulting rejected config).

**Bump rationale (MAJOR)**: `docs/maintenance/releasing.md` lists "Remove configuration parameters" as a MAJOR trigger — this release removes `length_min_ratio`/`length_max_ratio`/the `'length'` filter name and rejects `recall_threshold=0` (previously valid), both silent breaks for a deployment using them. Deliberate reversal of the v5.2.0 pattern (MINOR chosen over a MAJOR recommendation, DECISION-033) — here the documented criteria were matched literally. See DECISION-035.

**v6.0.1 — filter_pipeline validation fix + new standing rule**: `filter_pipeline` was never validated by `ConfigurationService` at all; a rejection fell through to a bare `False` with no field-level detail (the create path even cited `recall_threshold` as a misleading example). This was the single most likely v6.0.0 upgrade failure, since that release removed the `'length'` filter. Both create and update paths now return 400 naming the offending filter and listing valid ones; `VALID_FILTERS` hoisted to a shared constant (previously duplicated and able to disagree between the two validators). Drafting the fix surfaced a new **standing project rule** (DECISION-037): runtime messages must never include deprecation/removal commentary — that belongs in `CHANGELOG.md` only. 5 messages corrected (1 new, 4 pre-existing: session rehydration warning, `getPatterns()`/`getPatternsAsync()` `RuntimeError`s, `symbols_kb.update_one()` log warning, `/percept-data`/`/cognition-data` `warning` field). Now enforced by test (`test_unknown_filter_is_rejected_naming_the_filter`, fails on banned substrings like "deprecat"/"removed in"/a version number).

**Verification**: v6.0.0 — losslessness proven over 120,000 pattern/STM pairs (0 violations), mutation-checked tests, live audit-mode shadow run (360 dropped, 0 reachable), byte-identical prediction parity; full suite **659 passed / 3 skipped / 1 xfailed** (was 625); fresh-pull image verification confirmed `recall_bounds` present, `LengthFilter` absent, `max_length` at `r=0.1` correctly `114`. v6.0.1 — full suite **660 passed / 3 skipped / 1 xfailed**; fresh-pull verification confirmed the corrected messages and 0 `.pyc` shipped; live deployment confirmed `recall_threshold=0` and `filter_pipeline=['length']` both return 400 cleanly. Both releases' pre-release gates (ruff, bandit, pip-audit) clean.

**Self-caught process error**: v6.0.0's release notes as first published claimed the `filter_pipeline` rejection "named the filter" — asserted without testing, and false (see DECISION-035's Process Note). Corrected in the published GitHub release notes and `CHANGELOG.md` before this session ended; the real defect was fixed in v6.0.1. Logged as a new pattern in `project-manager/patterns.md`.

**Archive**: `planning-docs/completed/optimizations/2026-09-21-recall-safe-candidate-bound.md` (technical detail, unchanged), `planning-docs/completed/features/2026-09-21-kato-v6.0.0-release.md`, `planning-docs/completed/features/2026-09-21-kato-v6.0.1-release.md`. **Decisions**: DECISION-034, DECISION-035, DECISION-036, DECISION-037 in `planning-docs/DECISIONS.md`.

**Next task (top priority)**: ship to staging with `KATO_RECALL_BOUND_AUDIT=true` for 24h before fully trusting the bound against real corpus shapes no synthetic test anticipated, then turn audit off — see `project-manager/pending-updates.md`.
...
**Items**:
- ~~**Default `filter_pipeline` is still `[]`** — every prediction scans the node's whole corpus; all of `kato/filters/` is unreachable by default.~~ **DONE 2026-09-21, released same day as KATO v6.0.0** — resolved, but **not** via the route originally proposed here (fixing `LengthFilter`'s ratios). Instead a new lossless length+token-overlap predicate (`kato/filters/recall_bounds.py`) is applied intrinsically to the candidate query, on by default; `LengthFilter` itself was deleted outright as recall-unsafe (its fixed ratios discarded valid matches for any `r<=2/3`) rather than fixed. `filter_pipeline` itself remains `[]` by default — the bound is not a pipeline filter, deliberately, since the executor's exception-swallowing would turn a failure into an empty result set behind HTTP 200. See DECISION-034/DECISION-035 and `planning-docs/completed/optimizations/2026-09-21-recall-safe-candidate-bound.md`. Merged, released as v6.0.0, patched same day as v6.0.1 — deployed.
...
**Status**: **RESOLVED 2026-09-21** — see DECISION-034 and the "KATO v6.0.0 then v6.0.1 Release" entry under "Recently Completed" above. Resolved by a lossless length+token-overlap predicate pushed into ClickHouse, closer in spirit to option 3 below but combining it with a token-overlap clause (the length window alone was measured to keep 99.77% of candidates — worthless on its own). Merged and released as v6.0.0 same day, patched same day as v6.0.1 — deployed, nothing further open on this item besides the recommended staging audit soak (see below).
...
### Release Needed: `perf/recall-safe-candidate-bound` Is Complete and Verified But Unmerged — RESOLVED
**Priority**: P1 — complete, verified, ready work; nothing technically blocking it
**Status**: **RESOLVED 2026-09-21, same day as filed** — merged to `main` (`51f8213`) and released as **v6.0.0** (MAJOR), patched same day as **v6.0.1** (PATCH) after a `filter_pipeline` validation gap and a runtime-messaging rule violation were found post-release. Deployment stack now pinned to v6.0.1. See DECISION-035/DECISION-036/DECISION-037.
**Detail**: DECISION-034's recall-safe candidate bound was done on branch `perf/recall-safe-candidate-bound` (5 commits, full suite 659 passed / 3 skipped / 1 xfailed). **Note**: the recommended staging soak (`KATO_RECALL_BOUND_AUDIT=true` for 24h) was done neither before nor after this release — carried forward as its own top-priority follow-up item immediately below, since real corpus shapes were never exercised against the bound before it reached production.
**Files**: n/a (process item)
**Related**: DECISION-034, DECISION-035, DECISION-036; `planning-docs/completed/optimizations/2026-09-21-recall-safe-candidate-bound.md`, `planning-docs/completed/features/2026-09-21-kato-v6.0.0-release.md`, `planning-docs/completed/features/2026-09-21-kato-v6.0.1-release.md`.

---

### TOP PRIORITY: Staging Soak With `KATO_RECALL_BOUND_AUDIT=true` Before Fully Trusting the Bound on Real Corpus Shapes
**Priority**: P0 — promoted to top priority now that the bound is in production without this soak having been run first
**Status**: Open — identified 2026-09-21 during DECISION-034's work, still not done as of the v6.0.0/v6.0.1 releases shipping the same day
**Detail**: The recall-safe candidate bound's losslessness is proven mathematically and tested exhaustively against synthetic corpora, but no real training-data corpus has yet exercised it. Run with `KATO_RECALL_BOUND_AUDIT=true` in staging for 24h against real corpus shapes no synthetic test anticipated, watching for any `RECALL BOUND VIOLATION` log lines, then turn audit off once confirmed clean.
**Files**: n/a (operational task)
**Related**: DECISION-034, DECISION-035; `planning-docs/completed/optimizations/2026-09-21-recall-safe-candidate-bound.md`.
