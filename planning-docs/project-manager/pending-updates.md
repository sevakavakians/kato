...
## 2026-09-17 - Decision Needed: Commit the Deprecation-Warnings + Resource-Teardown Fixes?
**Issue**: A complete, verified body of work clearing `DeprecationWarning`s from the 5.1.1/5.1.2 dependency upgrade (`@app.on_event`→`lifespan`, redis `close()`→`aclose()`, `httpx2`/`anyio` floor raise) plus 3 latent resource-teardown bug fixes (session manager never shut down, concurrency reporter task unmanaged, `MetricsCacheManager` Redis client leak) sits entirely uncommitted in the working tree. Current local branch `perf/prediction-path-scaling` points at the same commit as `main` (`adc066d`) — no divergent history — so this is effectively uncommitted work directly on top of `main`.
**Impact**: Three real resource-leak bugs (confirmed via before/after shutdown-log evidence) remain unfixed in the committed codebase until this lands. Low urgency in practice (each leak only manifests on process shutdown/restart, not mid-request), but it is real, verified, ready work with nothing blocking it.
**Suggested Action**: Commit directly to `main`, or cut a dedicated branch and merge — same pattern used for Remediation Pass 1 (see the 2026-09-16 "Commit and Merge `chore/remediation-pass-1`?" resolved entry below). Decide the commit message split (deprecation cleanup vs. bug fixes could be one commit or two).
**Priority**: Medium — verified and ready, no known risk, but not urgent (leaks are shutdown-time only).
**Status**: Open
...
## 2026-09-11 - Release Needed: v5.0.2 Lacks the Prediction Segmentation Fix (now expanded — Remediation Pass 1 + DECISION-031 also unreleased)
**Issue**: `e0ee17d` (2026-09-11, DECISION-028) fixed a real correctness bug — prediction segmentation misattributed events (phantom `missing` symbols) when a symbol recurs across events, plus made the single-symbol fast path return event-structured fields. `34910a70` (2026-09-11, DECISION-029) fixed a second, related bug the first fix's own audit surfaced. **Update 2026-09-16**: the released image is now further behind — it also lacks the entire Remediation Pass 1 body of work (`df9a76a`/`7233155`: 7 live bug fixes, SQL-injection parameterization, CORS/store hardening) and DECISION-031 (`7bae726`: the prediction-ranking nondeterminism fix — arguably the most release-worthy item on this list, since it's a correctness bug, and the ProcessPoolExecutor performance fix). The currently released **v5.0.2** image includes none of this; the deployment stack runs an unreleased local `kato:latest` dev build with everything.
**Impact**: Anyone pulling `ghcr.io/sevakavakians/kato:5.0.2`/`:5.0`/`:5`/`:latest` is missing a growing list of correctness and security fixes, most notably the prediction-ranking nondeterminism fix (DECISION-031) and the SQL-injection parameterization (DECISION-030).
**Suggested Action**: Release a patch (or reconsider the bump size — DECISION-030/031's scope is broader than the previous patch releases in this project's history, though still no API contract change) via `./container-manager.sh patch`, bundling `e0ee17d`, `34910a70`, `df9a76a`/`7233155`, `8deab2c`, and `7bae726`.
**Priority**: Medium-High — the accumulated gap between `main` and the released image is now large enough (a real SQL-injection fix, a real nondeterminism fix, plus the older segmentation fixes) to warrant prioritizing this over further deferral.
**Status**: Open
