...
## 2026-09-21 - Decision Needed: Merge and Release `perf/recall-safe-candidate-bound`
...
**Suggested Action**: Decide whether to merge now or hold for further review; if merging, decide the version bump (this is a default-on behavior change to the candidate path, even though output is provably unchanged — likely MINOR per `docs/maintenance/releasing.md`'s "performance improvements" classification, but the new `recall_threshold=0` rejection is a breaking validation change for any caller currently passing 0, which may argue for a closer look at bump size). Before fully trusting the bound against real corpus shapes no synthetic test anticipates, consider a staging soak with `KATO_RECALL_BOUND_AUDIT=true` for 24h first (see DECISION-034).
