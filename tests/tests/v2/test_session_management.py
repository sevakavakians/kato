"""
Test Suite for KATO v2.0 Session Management Specification
...
class TestSessionIsolation:
    ...
    @pytest.mark.asyncio
    async def test_basic_session_isolation(self, kato_v2_client):
        """Test that two sessions maintain separate STMs without collision"""
        # Create two sessions
        session1 = await kato_v2_client.create_session(user_id="user1")
        session2 = await kato_v2_client.create_session(user_id="user2")
        
        # User 1 builds sequence A, B, C
        await kato_v2_client.observe_in_session(
            session1['session_id'], 
            {"strings": ["A"]}
        )
        await kato_v2_client.observe_in_session(
        ...
        # Verify User 1's STM
        stm1 = await kato_v2_client.get_session_stm(session1['session_id'])
        assert stm1['stm'] == [["A"], ["B"], ["C"]], \
            f"User 1 STM corrupted. Expected [['A'], ['B'], ['C']], got {stm1['stm']}"
        
        # Verify User 2's STM
        stm2 = await kato_v2_client.get_session_stm(session2['session_id'])
        assert stm2['stm'] == [["X"], ["Y"], ["Z"]], \
            f"User 2 STM corrupted. Expected [['X'], ['Y'], ['Z']], got {stm2['stm']}"
        
        # Cleanup
        await kato_v2_client.delete_session(session1['session_id'])
        await kato_v2_client.delete_session(session2['session_id'])
    
    @pytest.mark.asyncio
    async def test_concurrent_session_operations(self, kato_v2_client):
        """Test that concurrent operations on different sessions don't interfere"""
        sessions = []
        
        # Create 10 sessions
        for i in range(10):
            session = await kato_v2_client.create_session(user_id=f"user{i}")
        ...
        async def process_session(session_id: str, prefix: str):
            ...
            return await kato_v2_client.get_session_stm(session_id)
    ...
    @pytest.mark.asyncio
    async def test_session_with_vectors_and_emotives(self, kato_v2_client):
        ...
        pattern1 = await kato_v2_client.learn_in_session(session1['session_id'])
        pattern2 = await kato_v2_client.learn_in_session(session2['session_id'])
        ...
        pred1 = await kato_v2_client.get_session_predictions(session1['session_id'])
        pred2 = await kato_v2_client.get_session_predictions(session2['session_id'])
...
class TestSessionLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_session_creation(self, kato_v2_client):
        ...
        session_with_user = await kato_v2_client.create_session(
        ...
        session_with_meta = await kato_v2_client.create_session(
            metadata=metadata
        )
        assert session_with_meta['metadata'] == metadata
        
        # Session with custom TTL
        session_with_ttl = await kato_v2_client.create_session(
        ...
        for s in [session, session_with_user, session_with_meta, session_with_ttl]:
            await kato_v2_client.delete_session(s['session_id'])
    
    @pytest.mark.asyncio
    async def test_session_expiration(self, kato_v2_client):
        ...
        stm = await kato_v2_client.get_session_stm(session_id)
        assert stm['stm'] == []
        ...
        with pytest.raises(SessionNotFoundError) as exc:
            await kato_v2_client.get_session_stm(session_id)
        assert "expired" in str(exc.value).lower()
    
    @pytest.mark.asyncio
    async def test_session_cleanup(self, kato_v2_client):
        ...
        initial_count = await kato_v2_client.get_active_session_count()
        ...
        for session in sessions:
            with pytest.raises(SessionNotFoundError):
                await kato_v2_client.get_session_stm(session['session_id'])
        
        # Active count should be reduced
        final_count = await kato_v2_client.get_active_session_count()
        assert final_count == initial_count - 5
    
    @pytest.mark.asyncio
    async def test_session_extension(self, kato_v2_client):
        """Test extending session TTL"""
        # Create session with short TTL
        session = await kato_v2_client.create_session(ttl_seconds=5)
        session_id = session['session_id']
        ...
        # Extend session
        await kato_v2_client.extend_session(session_id, ttl_seconds=60)
        ...
        # Cleanup
        await kato_v2_client.delete_session(session_id)
...
class TestSessionPersistence:
    ...
    @pytest.mark.asyncio
    async def test_stm_persistence(self, kato_v2_client):
        ...
        # Clear STM
        await kato_v2_client.clear_session_stm(session_id)
    ...
    @pytest.mark.asyncio
    async def test_session_learn_and_predict(self, kato_v2_client):
        ...
        # Learn the pattern
        learn_result = await kato_v2_client.learn_in_session(session_id)
        ...
        predictions = await kato_v2_client.get_session_predictions(session_id)
...
class TestSessionLoadAndPerformance:
    ...
    async def test_many_concurrent_sessions(self, kato_v2_client):
        ...
        # Cleanup
        cleanup_tasks = [
            kato_v2_client.delete_session(s['session_id'])
            for s in sessions
        ]
    ...
    @pytest.mark.asyncio
    @pytest.mark.performance
    async def test_session_memory_limits(self, kato_v2_client):
        ...
        # Note: v2 doesn't enforce STM limits strictly, it may go slightly over
    ...
    @pytest.mark.asyncio
    @pytest.mark.stress
    async def test_session_stress_test(self, kato_v2_client):
...
class TestBackwardCompatibility:
    """Test backward compatibility with v1.0 API"""
    
    @pytest.mark.asyncio
    async def test_default_session_mode(self, kato_v2_client):
        """Test that v1.0 endpoints work with default session"""
        # Use v1.0 endpoint without session ID
        result = await kato_v2_client.observe_v1(
            {"strings": ["backward_compatible"]}
        )
        
        assert result['status'] == "okay"
        
        # Get STM using v1 endpoint
        stm = await kato_v2_client.get_stm_v1()
        ...
        # Clear using v1 endpoint
        await kato_v2_client.clear_stm_v1()
        
        # Verify cleared
        stm = await kato_v2_client.get_stm_v1()
        # Note: v1 mode might persist some state
    
    @pytest.mark.asyncio
    async def test_header_based_session_routing(self, kato_v2_client):
...
class TestSessionErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_invalid_session_id(self, kato_v2_client):
    ...
    @pytest.mark.asyncio
    async def test_session_limit_exceeded(self, kato_v2_client):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_session_modifications(self, kato_v2_client):
        ...
        async def observe_concurrent(num):
            return await kato_v2_client.observe_in_session(
                session_id,
...
class MockKatoV2Client:
    """Mock client for testing v2.0 session endpoints"""
    ...
    async def observe_v1(self, observation: Dict, headers: Dict = None) -> Dict:
        """v1.0 compatible observe"""
        session_id = None
    ...
    async def get_stm_v1(self) -> Dict:
        """v1.0 compatible get STM"""
        if "default-session" in self.sessions:
            return {"stm": self.sessions["default-session"]["stm"]}
        return {"stm": []}
    
    async def clear_stm_v1(self) -> None:
        """v1.0 compatible clear STM"""
        if "default-session" in self.sessions:
