"""
Actual test suite for KATO v2.0 Session Management
Tests real v2 endpoints with running services
"""
...
class TestBasicSessionOperations:
    """Test basic session operations against real v2 services"""
    
    @pytest.mark.asyncio
    async def test_session_creation_and_deletion(self, kato_v2_client):
        ...
        stm = await kato_v2_client.get_session_stm(session["session_id"])
        assert stm["stm"] == []
    ...
    @pytest.mark.asyncio
    async def test_basic_observation_in_session(self, isolated_session, kato_v2_client):
        """Test basic observation in a session"""
        session_id = isolated_session["session_id"]
        
        # Add observation
        result = await kato_v2_client.observe_in_session(
            session_id,
            {"strings": ["hello", "world"]}
        )
    ...
    @pytest.mark.asyncio
    async def test_session_stm_persistence(self, isolated_session, kato_v2_client):
    ...
    @pytest.mark.asyncio
    async def test_session_clear_stm(self, isolated_session, kato_v2_client):
...
class TestSessionIsolation:
    ...
    @pytest.mark.asyncio
    async def test_two_session_isolation(self, kato_v2_client):
...
class TestSessionWithVectors:
    ...
    @pytest.mark.asyncio
    async def test_session_with_multimodal_data(self, isolated_session, kato_v2_client):
        ...
        result = await kato_v2_client.observe_in_session(session_id, observation)
        assert "status" in result
...
class TestSessionPerformance:
    ...
    @pytest.mark.asyncio
    async def test_session_creation_performance(self, kato_v2_client):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_observations_performance(self, kato_v2_client):
        """Test performance with concurrent observations"""
        # Create session
        session = await kato_v2_client.create_session(user_id="perf_test")
        ...
        try:
            ...
            # Relaxed threshold - v2 has more overhead with session management
            assert ops_per_second > 10, f"Throughput too low: {ops_per_second:.1f} ops/sec"
            
            # Verify all observations were recorded
            stm = await kato_v2_client.get_session_stm(session_id)
            assert len(stm["stm"]) == total_ops
            
        finally:
            await kato_v2_client.delete_session(session_id)


# V1 compatibility tests removed per project requirement
# All tests now use v2 session-based APIs only
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_invalid_session_operations(self, kato_v2_client):
    ...
    @pytest.mark.asyncio
    async def test_expired_session_handling(self, kato_v2_client):
...
if __name__ == "__main__":
    # Run with: python -m pytest tests/tests/v2/test_session_management_actual.py -v
